...
- Vào Services -> EKS -> eksdemo -> Compute -> Node groups (eksdemo3-ng-public1)
...
eksctl create iamserviceaccount \
  ...
  --name=aws-load-balancer-controller \
  --role-name=AmazonEKSLoadBalancerControllerRole \
  --attach-policy-arn=arn:aws:iam::249062636362:policy/AWSLoadBalancerControllerIAMPolicy \
  --approve \
...
# Dùng lệnh này để tìm VPC-Id
eksctl get cluster eksdemo3 --region us-east-1 -o yaml
...
helm install aws-load-balancer-controller eks/aws-load-balancer-controller \
  ...
  --set vpcId=vpc-05eaae3ac18167456

# kiểm tra
kubectl get deployment -n kube-system aws-load-balancer-controller
```

### Cấp quyền đầy đủ cho dự án dùng được loadbalancer
...
# Reset để dùng quyền mới 
kubectl rollout restart deployment aws-load-balancer-controller -n kube-system

# Theo dõi log & xác nhận ALB đã được tạo
...
``` bash
nano resourcequota-frontend.yaml
...
``` bash
# Triển khai dự án
kubectl apply -f frontend.yaml -n frontend-resource
